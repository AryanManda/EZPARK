...
import { addVehicle, deleteVehicleById, getVehicles } from "./api/parkingApi";
...
export default function Vehicles({ userId }) {
  ...
  useEffect(() => {
    ...
    getVehicles(userId)
      ...
      .catch(() => {
        if (!mounted) return;
        setMsgType("error");
        setMsg("Unable to load vehicles right now.");
      });
  ...
  const addVehicleHandler = async () => {
    ...
    const exists = vehicles.find((v) => v.licensePlate.toUpperCase() === normalizedPlate);
    if (exists) return setMsg("A vehicle with this plate number already exists.");

    if (!userId) {
      setMsg("You must be logged in to add a vehicle.");
      return;
    }
    ...
    try {
      ...
      setMsgType("success");
      setMsg(`Vehicle ${normalizedPlate} added successfully!`);
      setPlate("");
      setMake("");
      setModel("");
    } catch (err) {
      setMsgType("error");
      setMsg(err.response?.data?.error || "Failed to add vehicle.");
    }
  };
  ...
  const deleteVehicle = async (id) => {
    if (!userId) {
      setMsgType("error");
      setMsg("You must be logged in to remove a vehicle.");
      return;
    }

    try {
      await deleteVehicleById(id, userId);
      setVehicles((prev) => prev.filter((v) => v.id !== id));
      setMsgType("success");
      setMsg("Vehicle removed successfully!");
    } catch (err) {
      setMsgType("error");
      setMsg(err.response?.data?.error || "Failed to remove vehicle.");
    }
  };
  ...
  return (
    <div style={{ maxWidth: "500px", margin: "0 auto", padding: "20px" }}>
      ...
      <div style={{ background: "#1e1e1e", padding: "20px", borderRadius: "10px", marginBottom: "20px" }}>
        ...
        <button onClick={addVehicleHandler} style={{
